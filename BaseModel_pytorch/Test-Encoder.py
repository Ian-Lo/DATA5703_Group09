...
###### RUN CALCULATION WITH LINEAR LAYER #######
# import model
...
model = Model(relative_path,
                ...
                structural_encoder_conv = structural_encoder_conv,
                cell_content_encoder_conv = cell_content_encoder_conv)
...
plt.plot(loss, label = 'Linear layer')


###### RUN CALCULATION WITH CONV LAYER #######

structural_encoder_conv = True
cell_content_encoder_conv = True
...
plt.plot(loss, label = 'Convolutional layer')
plt.title("Effect of using a convolutional layer or a linear layer in encoder")
plt.legend()
