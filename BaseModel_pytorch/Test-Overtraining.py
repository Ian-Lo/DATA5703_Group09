...
lrs = [0.001]*epochs#+[0.0005]*200 + [0.0002]*200 + [0.0001]*400
...
ax1.title.set_text("Test lambda tuning link")
...
for l in [0.5, 0.1, 0.01, 0.001]:
    ...
    model.load_checkpoint(file_path="Checkpoints/test_overtraining/checkpoint_099.pth.tar")
    ...
    ax1.set_ylabel('Loss')
#    ax1.plot(loss, label = 'Total loss, %.5f'%l)
#    ax1.plot(loss_s, label = 'Structural loss, %.5f'%l)
    ax1.plot(loss_cc, label = 'Cell loss, %.3f'%l)

ax1.legend()
fig.tight_layout()
plt.show()
plt.savefig('Figures/overtraining.png')
